...
describe('alert repository', () => {
  it('stores rules, evaluation state and triggers with cascade delete', async () => {
    const { AlertRepository } = await import('../electron/main/database/repositories/alertRepository')
    const dir = mkdtempSync(join(tmpdir(), 'ci-db-'))
    const db = openDatabase(join(dir, 'a.db'))
    const repo = new AlertRepository(db)
    const r = repo.create({ assetId: 'bitcoin', kind: 'price', direction: 'above', threshold: 100000, mode: 'once', note: 'n' }, 'BTC')
    expect(r.enabled).toBe(true)
    expect(r.armed).toBe(false)
    repo.saveState({ ...r, armed: true, enabled: false, lastTriggeredAt: 5, triggerCount: 1 })
    const saved = repo.get(r.id)
    expect(saved.armed).toBe(true)
    expect(saved.enabled).toBe(false)
    expect(saved.triggerCount).toBe(1)
    expect(repo.setEnabled(r.id, true).armed).toBe(false)
    repo.addTrigger({ alertId: r.id, symbol: 'BTC', kind: 'price', message: 'm', value: 1, triggeredAt: 5 })
    expect(repo.listTriggers()).toHaveLength(1)
    const u = repo.update(r.id, { assetId: 'bitcoin', kind: 'rsi', direction: 'below', threshold: 30, mode: 'repeating', note: '' }, 'BTC')
    expect(u.kind).toBe('rsi')
    repo.delete(r.id)
    expect(repo.listTriggers()).toHaveLength(0)
    db.close()
    rmSync(dir, { recursive: true, force: true })
  })
})
