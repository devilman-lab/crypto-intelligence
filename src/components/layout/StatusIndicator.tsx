...
export function StatusIndicator() {
  const { online, degraded, lastMarketUpdateAt, reason } = useConnectivityStore()
  ...
  const detail = online
    ? `${lastMarketUpdateAt ? `Updated ${formatRelativeTime(lastMarketUpdateAt)}` : 'Waiting for data'}${degraded ? ' · retrying' : ''}`
    : 'Showing cached data'
  ...
  return (
    <Tooltip content={reason ?? (online ? 'Connected to the market data provider.' : 'No internet connection. Portfolio, journal and cached data remain available.')}>
      <div className="flex items-center gap-2 rounded-md border border-border bg-surface-2 px-2.5 py-1">
        <span className={cn('h-2 w-2 rounded-full', !online ? 'border border-fg-subtle' : degraded ? 'bg-warning shadow-[0_0_6px_var(--warning)]' : 'bg-positive shadow-[0_0_6px_var(--positive)]')} />
